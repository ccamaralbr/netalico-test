    ...
    _updateAddToCart: function (evt) {
      ...
      if (variant) {
        if (variant.available) {
          ...
          // Changing color to add to cart button while in Available state  @Celso
          this.addToCart.style.backgroundColor = "#001858";
          //
          addToCartText.innerHTML = theme.strings.addToCart;
          // Bringing generated inventory info to JS process custom button messages @Celso
          var inventory_qty = inv_qty[variant.id];

          //// Custom quantity-stepper button messages @Celso
          if (inventory_qty > 9) {
            document.querySelector(
              ".product-form.product-form-product-template > label"
            ).textContent = "In stock & ready to ship";
          } else {
            document.querySelector(
              ".product-form.product-form-product-template > label"
            ).textContent = "hurry!";
          ...
          // Changing color to add to cart button while in OOS state @Celso
          this.addToCart.style.backgroundColor = "#6E6E6E";
          //
          ...
          document.querySelector(
            ".product-form.product-form-product-template > label"
          ).textContent = theme.strings.soldOut;
        }
      } else {
        // The variant doesn't exist, disable submit button and change the text.
        this.addToCart.setAttribute("aria-disabled", true);
        this.addToCart.setAttribute("aria-label", theme.strings.unavailable);
        // Changing color to add to cart button while in Unavailable state (Same as OOS)  @Celso
        this.addToCart.style.backgroundColor = "#6E6E6E";
...
// Custom snippet for quantity selector buttons @Celso
